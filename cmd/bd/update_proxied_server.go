...
func buildUpdateSpecForIssue(current *types.Issue, in *updateInput) (domain.UpdateSpec, error) {
	...
	if len(in.setMetadata) > 0 || len(in.unsetMetadata) > 0 || len(in.unsetMetaPrefix) > 0 {
		merged, err := applyMetadataEdits(current.Metadata, in.setMetadata, in.unsetMetadata, in.unsetMetaPrefix)
		if err != nil {
