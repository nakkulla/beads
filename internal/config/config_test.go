...
func TestGetExternalDatabases(t *testing.T) {
	err := Initialize()
	if err != nil {
		t.Fatalf("Initialize() returned error: %v", err)
	}
	...
	// Test default (empty map)
	got := GetExternalDatabases()
	if got == nil {
		t.Error("GetExternalDatabases() returned nil, want empty map")
	}
	if len(got) != 0 {
		t.Errorf("GetExternalDatabases() = %v, want empty map", got)
	}

	// Test with Set
	Set("external_databases", map[string]string{
		"beads":         "beads_prod",
	...
	got = GetExternalDatabases()
	if len(got) != 2 {
		t.Errorf("GetExternalDatabases() has %d items, want 2", len(got))
	}
	if got["beads"] != "beads_prod" {
		t.Errorf("GetExternalDatabases()[beads] = %q, want \"beads_prod\"", got["beads"])
	}
	if got["other-project"] != "other_project_db" {
		t.Errorf("GetExternalDatabases()[other-project] = %q, want \"other_project_db\"", got["other-project"])
	}
}

func TestGetExternalDatabasesFromConfig(t *testing.T) {
	// Create a temporary directory for config file
	tmpDir := t.TempDir()

	// Create a config file with external_databases
	configContent := `
external_databases:
  beads: beads_prod
  other-project: other_project_db
...
`
	...
	// Test that external_databases is loaded correctly
	got := GetExternalDatabases()
	if len(got) != 3 {
		t.Errorf("GetExternalDatabases() has %d items, want 3", len(got))
	}
	if got["beads"] != "beads_prod" {
		t.Errorf("GetExternalDatabases()[beads] = %q, want \"beads_prod\"", got["beads"])
	}
	...
	if got["other"] != "other_db" {
		t.Errorf("GetExternalDatabases()[other] = %q, want \"other_db\"", got["other"])
	}
}
