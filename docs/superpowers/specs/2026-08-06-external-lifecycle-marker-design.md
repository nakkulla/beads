...
- 접근자 `(c *Config) HasExternalServerLifecycle() bool`: trim + lower 정규화 후
  **비어있지 않은 모든 값에 대해 true**. 오타·미래 값이 Owned로 조용히 굴러떨어져
  섀도 서버를 fork하는 것보다, auto-start 억제(접속 에러로 표면화)가 안전하다는
  fail-safe 원칙. 미인식 값 진단은 doctor가 담당(§4).
...
- 메타데이터 기록 블록(1220-1233)의 `!usesProxiedServer()` 분기: `serverPort != 0`일 때
  `cfg.DoltServerLifecycle = configfile.DoltServerLifecycleExternal`을 port와 함께 기록.
- `initTimeCloneConfig`(2592-2608, 원격 부트스트랩 경로)에도 동일 조건 추가.
- 마커를 쓰지 않는 경로(명시): `--server` 단독(포트 없음 = Owned auto-start 리그),
  `BEADS_DOLT_SERVER_MODE=1` env(런타임 최우선 권위로 이미 해석됨), shared-server init.
...
- 등록: doctor 체크 목록(카테고리 `CategoryData`)과 `cmd/bd/doctor_fix.go`의 체크명
  디스패치에 추가하되 **port-drift 체크보다 앞 순서** — `bd doctor --fix` 한 번에
  핀 → (drift가 있으면) 키 제거가 이어지도록.
...
| 마커 미인식 값 (예: 오타) | External (fail-safe) + doctor 경고 |
...
1. **servermode 판별** (`internal/doltserver/servermode_forconfig_test.go`):
   마커→External(port 무관), 미인식 값→External(fail-safe), embedded+마커→Embedded,
   마커 부재+port→External(레거시 폴백 유지), 마커 부재+port 부재→Owned.
2. **init 수용** (`cmd/bd/init_server_mode_acceptance_test.go` 계열):
   `--server --server-port X` → metadata.json에 마커 기록;
   `--server` 단독 → 마커 없음 + Owned 유지; 재init 시 마커 보존.
