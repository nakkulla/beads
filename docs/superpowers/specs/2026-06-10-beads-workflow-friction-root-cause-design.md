...
2. server-mode, embedded Dolt, JSONL auto-import 경계에서 stale `issues.jsonl`가 현재 DB state를 덮거나 반복 auto-import되는 조건을 제거한다.
3. `bd dolt push` dangling refs 실패가 발생하는 경로를 재현하거나, 이미 known-limitation인 경우 CLI가 더 안전하게 classify/recover하도록 만든다.
4. plan-backed child Bead seeding에 필요한 `bd create --id`, `--parent`, dependency/readback/reuse 흐름을 idempotent하게 만든다.
5. `.beads/.~issues.jsonl.*` 같은 editor/temp JSONL 파일이 import/export/sync source로 잘못 취급되지 않음을 보장한다.
6. 수정 후 전역 `bd`를 이 repo build로 다시 설치하고, active `bd`가 Homebrew가 아니라 local build임을 검증한다.
...
사용자가 관찰한 rollback류 실패는 stale `.beads/issues.jsonl`가 DB empty 판정 또는 fallback import path를 통해 현재 Dolt rows 위로 재적용되는 경우와 관련될 수 있다. 현재 코드에는 size/mtime stamp, `GetStatistics` emptiness guard, `ConflictSkip`, stale `UpdatedAt` filter가 있다. 이 방어가 embedded/server-mode 양쪽에서 실제 CLI 시나리오를 충분히 커버하는지 검증해야 한다.

#### 구현 방향

- `cmd/bd/auto_import_upgrade_test.go` 또는 regression harness에 다음 시나리오를 추가한다.
  1. stale `issues.jsonl`에 오래된 issue state가 있다.
...
- server-mode fallback path를 unit substitute로만 검증하지 말고, 가능한 범위에서 CLI-level fixture도 둔다.
- `GetStatistics`가 temporary bootstrap/import state에서 false empty를 반환할 수 있는지 확인한다.
- false empty가 재현되면, auto-import 진입 조건을 다음 중 하나 이상으로 강화한다.
  - DB directory/schema initialization marker와 issue table existence를 함께 확인한다.
  - `issues.jsonl`보다 최신 Dolt commit 또는 write marker가 있으면 auto-import 금지한다.
  - auto-import stamp를 성공/실패 모두에 대해 더 명시적으로 기록하되, empty DB recovery가 필요한 fresh clone path는 유지한다.

#### 수용 기준

- stale JSONL이 최신 DB row를 덮지 않는 regression test가 추가된다.
- embedded path와 fallback path 중 실제 취약한 쪽이 확인되며, 수정 후 테스트가 통과한다.
...
- CLI 자체의 `create --id --parent` 동작을 먼저 검증한다.
  - explicit child ID가 parent prefix와 함께 유효한지
  - parent-child dependency가 생성되는지
  - 같은 ID로 재실행하면 duplicate error가 명확히 나오는지
- helper/script가 아니라 CLI primitive가 불안정하면 `cmd/bd/create.go` 또는 validation layer를 수정한다.
...
- idempotent pattern은 read-before-create이다.
  ...
  3. 없으면 create한다.
  4. create output parse 실패 시 즉시 두 번째 create를 하지 않고 independent readback으로 이미-created 여부를 확인한다.
...
`.beads/.~issues.jsonl.*` 같은 editor/temp file은 import path 기본값과 다르므로 직접 import되지는 않아야 한다. 하지만 cleanup, glob, sync helper가 잘못된 파일을 source로 삼으면 readback 혼란을 만들 수 있다.

#### 구현 방향

- `configuredImportJSONLPath`가 정확히 `.beads/issues.jsonl`만 선택함을 unit test로 고정한다.
...
2. `.beads/issues.jsonl`가 존재하고 non-empty인지 확인한다.
3. auto-import stamp와 DB state를 확인한다.
4. DB가 진짜 empty이고 recovery가 필요한 경우에만 import한다.
5. import는 insert-if-new/stale-skip semantic으로 수행한다.
6. import 후 commit/stamp가 기록된다.
7. 이후 일반 write/read는 stale JSONL을 다시 적용하지 않는다.
...
3. 없을 때만 `bd create --id ... --parent ...`를 수행한다.
...
- **false empty DB**: empty 판정이 불확실하면 auto-import하지 않고 warning 또는 recovery guidance를 출력한다.
...
- Auto-import:
  ...
  - 신규 stale JSONL vs latest Dolt row regression
- Import/readback:
